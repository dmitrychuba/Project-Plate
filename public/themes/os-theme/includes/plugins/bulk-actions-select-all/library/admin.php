...
class BASA_Admin {
    ...
    public function scripts($hook) {
        ...
        if (($hook == 'edit.php' || $hook == 'edit-comments.php') && !empty($wp_list_table)) {
    ...
    public function handle_bulkactions($action, $result) {
        ...
        if (!$result || ($action != 'bulk-posts' && $action != 'bulk-comments')) {
            return;
        }
        ...
        if (!in_array($wp_list_table->current_action(), array('trash', 'untrash', 'delete', 'approve', 'unapprove', 'spam', 'export')) || (empty($_REQUEST['post']) && empty($_REQUEST['delete_comments']))) {
            return;
        }
        ...
        if (!empty($_REQUEST['delete_comments'])) {
            $_REQUEST['delete_comments'] = get_comments(array('fields' => 'ids'));
        } else {
